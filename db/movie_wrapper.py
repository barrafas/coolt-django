...
genres_list = requests.get(
    'http://api.themoviedb.org/3/genre/movie/list?api_key=b33b8cf81183cc38fb69365485a59447').json()['genres']
...
for page in range(1, 6):
    top_movies = requests.get(
        f'https://api.themoviedb.org/3/movie/popular?language=pt-BR&page={page}&api_key=b33b8cf81183cc38fb69365485a59447').json()
    for movie in top_movies['results']:
        top_ids.add(int(movie['id']))
...
# Itera sobre cada id e captura/adiciona os dados do filme ao df
for id in top_ids:
    # Captura o json
    movie = requests.get(
        f'https://api.themoviedb.org/3/movie/{id}?language=pt-BR&api_key=b33b8cf81183cc38fb69365485a59447').json()
    try:
        # Filmes podem não ter título em inglês
        ...
        img_url = 'https://image.tmdb.org/t/p/original' + \
            movie['poster_path'] if movie['poster_path'] else ''
        avg_rating = float(movie['vote_average'])
        qtd_rating = int(movie['vote_count'])
